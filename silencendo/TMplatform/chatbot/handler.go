...
func (h *Handler) HandleWithMatch(ctx context.Context, match IntentMatch) (handled bool, reply string, err error) {
	...
	case IntentShowProject:
		...
		return handled, h.formatProjectDetails(ctx, details), nil
	}
...
func (h *Handler) formatProjectDetails(ctx context.Context, details models.ProjectDetails) string {
	const taskLimit = 50
	...
	if len(details.Stages) == 0 {
		...
		for _, s := range sorted {
			resp := h.userLabel(ctx, s.ResponsibleID)
	...
	for _, t := range details.Tasks {
		key := ""
		if t.StageID != nil {
			key = strings.TrimSpace(*t.StageID)
		}
	...
	stageOrder = append(stageOrder, "") // unassigned
	...
	for _, stageID := range stageOrder {
		...
		for _, t := range tasks {
			...
			assignee := h.userLabel(ctx, t.AssigneeID)
...
func (h *Handler) userLabel(ctx context.Context, userID *string) string {
	...
	name, err := h.Projects.GetUserNameByID(ctx, id)
