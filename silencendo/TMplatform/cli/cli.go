...
type CLIInterface struct {
	ctxManager        *botcontext.ContextManager
	ctxCommands       *botcontext.ContextCommands
...
func NewCLIInterface() *CLIInterface {
	ctxManager := botcontext.NewContextManager()
	ctxCommands := botcontext.NewContextCommands(ctxManager)
...
// HandleMessage processes a single message without reading from stdin.
// Returns true when the caller should exit the loop (e.g., /exit or /quit).
...
// SourceManager exposes the active source manager for routing decisions.
func (c *CLIInterface) SourceManager() *sources.SourceManager {
	return c.sourceManager
}

// IntentDetector exposes the document intent scoring function for routing decisions.
func (c *CLIInterface) IntentDetector() func(string) string {
	return c.detectIntent
}
...
// shouldRouteToProject catches project-style intents before document edits to prevent accidental file rewrites.
func (c *CLIInterface) shouldRouteToProject(instruction string) (chatbot.IntentMatch, bool) {
	match := chatbot.DetectIntent(instruction)

	switch match.Intent {
	case chatbot.IntentCreateProject, chatbot.IntentAddStagesAndTasks, chatbot.IntentAssignResponsible, chatbot.IntentListProjects, chatbot.IntentShowProject:
		return match, true
	}

	lower := strings.ToLower(instruction)
	projectKeywords := []string{"project", "stage", "task"}
	actionKeywords := []string{"create", "add", "assign", "responsible", "owner", "manager"}
...
func (c *CLIInterface) handleEditRequest(instruction string) error {
	if match, isProject := c.shouldRouteToProject(instruction); isProject {
		fmt.Println("This request looks like project management, so I will not edit any documents.")
		if strings.TrimSpace(match.ProjectTitle) == "" {
			fmt.Println("If you want to manage a project, say 'create project <name>' or use the /project commands.")
		} else {
			fmt.Printf("If you want to manage project '%s', say 'create project %s' or use the /project commands.\n", match.ProjectTitle, match.ProjectTitle)
		}
		fmt.Println("If you intended to edit a document, please rephrase with the document and change you want.")
		return nil
	}
