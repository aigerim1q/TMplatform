  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (CHATBOT_API_URL) {
      try {
        ...
        const data = (await res.json()) as { reply: string; exit?: boolean; active_project_id?: string; active_project_title?: string };
        setChatSession({
        ...
        setMessages((prev) => [...prev, { type: 'ai', text: 'Error: ' + (err instanceof Error ? err.message : String(err)) }]);
      }
      setIsLoading(false);
      return;
    }
    ...
    setTimeout(() => {
      let aiResponse: { type: string; text: string; project?: string } | null = null;
      if (userMessage.includes('shyraq')) {
        aiResponse = { type: 'ai', text: 'Вот информация о проекте Shyraq:', project: 'shyraq' };
      } else if (userMessage.includes('ansau')) {
        aiResponse = { type: 'ai', text: 'Вот информация о проекте Ansau:', project: 'ansau' };
      } else if (userMessage.includes('dariya')) {
        aiResponse = { type: 'ai', text: 'Вот информация о проекте Dariya:', project: 'dariya' };
      } else {
        aiResponse = { type: 'ai', text: 'Я помогу тебе! Напиши название проекта, например "Shyraq", "Ansau" или "Dariya".' };
      }
