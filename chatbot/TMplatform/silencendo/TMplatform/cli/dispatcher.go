...
type Dispatcher struct {
	...
	state           *SessionState
	Out             io.Writer // optional; when set, all output goes here (e.g. for API capture)
}
...
// DispatchAndCapture runs one dispatch with output captured to a buffer. Used by the HTTP API.
func (d *Dispatcher) DispatchAndCapture(ctx context.Context, input string) (reply string, exit bool, err error) {
	var buf bytes.Buffer
	prev := d.Out
	d.Out = &buf
	defer func() { d.Out = prev }()
	exit, err = d.Dispatch(ctx, input)
...
func (d *Dispatcher) Dispatch(ctx context.Context, input string) (bool, error) {
	...
	if (d.state == nil || strings.TrimSpace(d.state.ActiveProjectID) == "") && isProjectScopedInput(lower) {
		...
		d.println(noActiveProjectMessage())
		return false, nil
	}
	...
	case IntentKnowledge:
		if route.LLMFailed {
			d.println("DeepSeek couldn't understand that request. Please rephrase.")
			return false, nil
		}
...
func (d *Dispatcher) handlePending(ctx context.Context, input string) (bool, error) {
	...
	d.println("Please reply with 1 (project), 2 (document), or 'cancel'.")
	return false, nil
}
...
func (d *Dispatcher) handlePendingAssignment(ctx context.Context, input string) (bool, error) {
	...
	if (len(pending.Assignees) > 0 && (pending.TaskStableID != 0 || (pending.TaskPosStage != 0 && pending.TaskPosIndex != 0))) ||
		...
		if err != nil {
			d.printf("Error executing assignment: %v\n", err)
	...
	if pending.WaitingFor == "assignees" {
		if pending.TaskStableID != 0 {
			d.printf("Who should I assign to task #%d?\n", pending.TaskStableID)
		} else if pending.TaskPosStage != 0 && pending.TaskPosIndex != 0 {
			d.printf("Who should I assign to task %d-%d?\n", pending.TaskPosStage, pending.TaskPosIndex)
		} else {
			d.println("Who should I assign to which task?")
		}
	} else { // waiting for task reference
		d.println("Which task should I assign them to? Use task 4-2 or #8.")
	}
...
func (d *Dispatcher) handlePendingDeletion(ctx context.Context, input string) (bool, error) {
	...
	if lower == expected {
		...
		d.printf("🗑️ Project '%s' deleted.\n", deleted.Title)
		return true, nil
	}

	d.printf("To confirm deletion, reply with 'delete %s' or 'cancel'.\n", title)
	return true, nil
}
...
func (d *Dispatcher) dispatchWithForced(ctx context.Context, input string, forced IntentType, projectMatch *chatbot.IntentMatch) (bool, error) {
	...
	case IntentProject:
		...
		d.println("I couldn't understand that project request. Try /project list, /project use <number>, or /project current.")
		return false, nil
	case IntentDocument:
...
func (d *Dispatcher) handleProjectSlashCommand(ctx context.Context, args []string) error {
	...
	case "use", "set", "select", "выбрать", "использовать", "выбор":
		if len(args) < 2 {
			d.println("Usage: /project use <name|id|number>")
			return nil
		}
	...
	case "current", "active", "текущий", "активный":
		if d.state != nil && strings.TrimSpace(d.state.ActiveProjectID) != "" {
			...
			if handled {
				...
				d.println(d.describeCurrentProject())
			}
			return nil
		}
		d.println(noActiveProjectMessage())
	...
	case "clear", "сброс", "очистить":
		...
		d.println("Active project cleared.")
		return nil
	case "set-status", "status", "setstatus", "статус":
		if len(args) < 3 {
			d.println("Usage: /project set-status <active|inactive> <name|id|number>")
			return nil
		}
	...
	case "delete", "remove", "удали", "удалить":
		if len(args) < 2 {
			d.println("Usage: /project delete <name|id|number>")
			return nil
		}
		target := strings.Join(args[1:], " ")
		return d.requestProjectDeletion(ctx, target)
	default:
		d.println("Unknown /project subcommand. Try: help, list, use, current, clear, set-status, delete.")
		return nil
	}
}

func (d *Dispatcher) printProjectHelp() {
	d.println("/project help - Show this help")
	d.println("/project list [--verbose] - List your projects")
	d.println("/project use <name|id|number> - Set active project")
	d.println("/project current - Show active project")
	d.println("/project clear - Clear active project")
	d.println("/project set-status <active|inactive> <name|id|number> - Update status")
	d.println("/project delete <name|id|number> - Soft delete a project (confirm required)")
}
...
func (d *Dispatcher) printProjectList(ctx context.Context, verbose bool) error {
	...
	if len(projects) == 0 {
		d.println("You have no projects yet. Create one with 'create project <name>'.")
		return nil
	}
	...
	d.println("📁 Your projects:")
	d.println(strings.Join(lines, "\n"))
	return nil
}
...
func (d *Dispatcher) updateProjectStatus(ctx context.Context, status, target string) error {
	if status != "active" && status != "inactive" && status != "активный" && status != "неактивный" {
		d.println("Status must be 'active' or 'inactive'.")
		return nil
	}
	...
	if d.state != nil && strings.TrimSpace(d.state.ActiveProjectID) == updated.ID {
		d.state.ClearActiveProject()
		d.printf("Project %s marked inactive and current selection cleared.\n", updated.Title)
		return nil
	}

	d.printf("Project %s status set to %s. Current session project remains %s.\n", updated.Title, status, d.currentProjectLabel())
	return nil
}
...
func (d *Dispatcher) requestProjectDeletion(ctx context.Context, target string) error {
	if d.state != nil && strings.TrimSpace(d.state.ActiveProjectID) != "" {
		...
		d.printf("⚠️ Delete project '%s'? Reply 'delete %s' to confirm or 'cancel'.\n", project.Title, project.Title)
		return nil
	}
...
func (d *Dispatcher) resolveProjectSelection(ctx context.Context, target string) (models.Project, error) {
	...
	if normalizedTarget == "" {
		d.println("Please provide a project name, number, or id prefix.")
	...
	if idx, errConv := strconv.Atoi(normalizedTarget); errConv == nil {
		if idx <= 0 {
			d.println("Invalid project number. Use /project list and pick a listed number.")
			return models.Project{}, nil
		}
		if idx <= len(projects) {
	...
	if len(titleMatches) > 1 {
		d.println("Multiple projects match that title. Use /project list and pick a number.")
		return models.Project{}, nil
	}
	...
	if len(prefixMatches) > 1 {
		d.println("Multiple projects match that id prefix. Use /project list --verbose to choose a number.")
		return models.Project{}, nil
	}

	d.println("Project not found. Use /project list to see available projects.")
	return models.Project{}, nil
}
