...
## Running with HTTP API (connect frontend)

To run the chatbot with the HTTP API so the main frontend can connect:

**From workspace root `TMplatform`**, run one of:

```bash
./run_chatbot_http.sh
```

Or manually (run each line separately; do not paste the comment):

```bash
cd silencendo/TMplatform
export CHATBOT_HTTP=1
export CHATBOT_HTTP_PORT=8080
go run .
```

Then in another terminal, run the frontend with the backend URL:

```bash
cd ..
export CHATBOT_API_URL=http://localhost:8080
npm run dev
```
